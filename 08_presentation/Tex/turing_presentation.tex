...
\begin{frame}
	...
	\begin{block}{Definition}
		...
		\begin{itemize}
			\item $\Sigma$ \textit{terminal symbols}
			\item V \textit{variables} with V $\cap$ $\Sigma$ = $\emptyset$
			\item P $\subseteq$ $(V \cup \Sigma)^+$ $\times$ $(V \cup \Sigma)^*$ \textit{production rules}
			\item S $\in$ V \textit{start symbol}
		\end{itemize}
	\end{block}
\end{frame}
...
\begin{frame}
	...
	\begin{exampleblock}{Example}
		\begin{equation*}
			\begin{aligned}
				G ={} ( & \{\text{the man}, \text{the book}, \text{took}\}, \\ 
				& \{S, NP, VP, V\}, P, S)
			\end{aligned}
		\end{equation*}
		\begin{equation*}
...
\begin{frame}
	...
	\begin{itemize}
		\item BNF generates exactly the context free languages \\
		\item EBNF has Exceptions (Rule - Exception)  \\ $\to$ not context free
	\end{itemize}
\end{frame}
...
\begin{frame}[t]
	...
	\only<2>{
	\begin{proofs}<2->
		Let $L_1 = \{a^nb^na^m | n,m\geq0\}$ be generated by \\ $\langle L_1 \rangle ::=\langle X \rangle\langle A \rangle$ \\  $\langle X \rangle ::=a \langle X \rangle b | \varepsilon$ \\ $\langle A \rangle ::= \langle A \rangle a | \varepsilon$ \vadjust{\vspace{10pt}}\nolinebreak\hspace{\fill}\linebreak
		Let $L_2 = \{a^nb^ma^m | n,m\geq0\}$ be generated by  \\$\langle L_2 \rangle ::=\langle A \rangle\langle X \rangle$ \\ $\langle X \rangle ::=a \langle X \rangle b | \varepsilon$ \\ $\langle A \rangle ::= \langle A \rangle a | \varepsilon$ \\
	\end{proofs}}
	\only<3>{
	\begin{proofs}<3->[\proofname\ (Cont.)]
		Now show $L_3 = L_1 \cap L_2 = \{a^nb^na^n | n\geq0\}$ not context free by contradiction. \vadjust{\vspace{10pt}}\nolinebreak\hspace{\fill}\linebreak
...
\begin{frame}
	...
	\begin{itemize}
		\item Block scope
		\item Call-by-value and call-by-name parameter passing
...
\begin{frame}[fragile] % Need to use the fragile option when verbatim is used in the slide
	...
	\begin{quote}{Peter Naur, Formalization in Program Development}
